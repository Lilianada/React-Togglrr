...
interface OffCanvasOverlayBottomProps {
  isOpen: boolean
  toggle: () => void
...
export function OffCanvasOverlayBottom({
  isOpen,
  toggle,
  children,
      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            animate={{ y: 0, boxShadow: "0 -10px 30px rgba(0,0,0,0.3)" }}
            exit={{ y: "100%", boxShadow: "0 0 0 rgba(0,0,0,0)" }}
            transition={{ type: "spring", stiffness: 300, damping: 30 }}
            className="fixed left-0 bottom-0 right-0 z-60 bg-background outline-none focus-visible:ring-2 focus-visible:ring-primary/70"
            style={{ height }}
            role="dialog"
            aria-modal="true"
